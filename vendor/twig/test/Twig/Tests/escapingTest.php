            ...
            if ($chr >= 0x30 && $chr <= 0x39 || $chr >= 0x41 && $chr <= 0x5A || $chr >= 0x61 && $chr <= 0x7A) {
                ...
                if (in_array($literal, $immune)) {
                    ...
                    $this->assertNotEquals(
                            $literal, twig_escape_filter($this->env, $literal, 'js'), "$literal should be escaped!");
                }
            }
        }
    }
        ...
        for ($chr = 0; $chr < 0xFF; ++$chr) {
            if ($chr >= 0x30 && $chr <= 0x39 || $chr >= 0x41 && $chr <= 0x5A || $chr >= 0x61 && $chr <= 0x7A) {
                ...
                if (in_array($literal, $immune)) {
                    ...
                    $this->assertNotEquals(
                            $literal, twig_escape_filter($this->env, $literal, 'html_attr'), "$literal should be escaped!");
                }
            }
        }
    }
    ...
    public function testCssEscapingEscapesOwaspRecommendedRanges() {
        ...
        for ($chr = 0; $chr < 0xFF; ++$chr) {
            if ($chr >= 0x30 && $chr <= 0x39 || $chr >= 0x41 && $chr <= 0x5A || $chr >= 0x61 && $chr <= 0x7A) {
                ...
                $literal = $this->codepointToUtf8($chr);
                $this->assertNotEquals(
                        $literal, twig_escape_filter($this->env, $literal, 'css'), "$literal should be escaped!");
            }
        }
    }
