...
abstract class Twig_Node_Expression_Call extends Twig_Node_Expression {
    ...
    protected function getArguments($callable, $arguments) {
            ...
            if (array_key_exists($name, $parameters)) {
                ...
                if (count($missingArguments)) {
                    throw new Twig_Error_Syntax(sprintf(
                            'Argument "%s" could not be assigned for %s "%s(%s)" because it is mapped to an internal PHP function which cannot determine default value for optional argument%s "%s".', $name, $callType, $callName, implode(', ', $names), count($missingArguments) > 1 ? 's' : '', implode('", "', $missingArguments))
                    );
                }
        ...
        if (!empty($parameters)) {
            ...
            throw new Twig_Error_Syntax(sprintf(
                    'Unknown argument%s "%s" for %s "%s(%s)".', count($parameters) > 1 ? 's' : '', implode('", "', array_keys($parameters)), $callType, $callName, implode(', ', $names)
